...
function synthesizeBrief(text, sourceIds, kind, ctx = {}) {
  ...
  return {
    headline: sentences[0] ? (sentences[0].length > 160 ? sentences[0].slice(0, 160).replace(/\s+\S*$/, "") + "…" : sentences[0].replace(/[.!?]+$/, "")) : "Intention to be written",
  ...
  return (
    <div style={{ background: origin ? T.panel : T.panel2, border: `1px solid ${origin ? `${T.accent}44` : T.line}`, borderRadius: 12, padding: "14px 16px", marginBottom: origin ? 18 : 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
        ...
        {origin ? <Chip color={T.accent}>Read-only copy · from {kind.fromShort}</Chip> : <Chip color={T.ok}>Synthesis</Chip>}
      ...
      <div style={{ fontSize: 13, fontWeight: 700, color: T.ink, lineHeight: 1.45, marginBottom: 10 }}>{s.headline}{s.headline.endsWith("…") ? "" : "."}</div>
